...
@app.route('/api/beta/monte-carlo', methods=['POST'])
@login_required
def api_beta_monte_carlo():
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403
    ...
    result = run_monte_carlo_simulation(tickers, weights)
    return jsonify(result)

@app.route('/api/beta/crypto-sentiment')
@login_required
def api_beta_crypto_sentiment():
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403
...
@app.route('/api/beta/insider/<ticker>')
@login_required
def api_beta_insider(ticker):
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403
    return jsonify(get_insider_sentiment(ticker))

@app.route('/api/beta/analysts/<ticker>')
@login_required
def api_beta_analysts(ticker):
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403
    return jsonify(get_analyst_ratings(ticker))

@app.route('/api/beta/fundamentals/<ticker>')
@login_required
def api_beta_fundamentals(ticker):
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403
    return jsonify(calculate_piotroski_f_score(ticker))

@app.route('/api/beta/options/<ticker>')
@login_required
def api_beta_options(ticker):
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403
...
@app.route('/api/beta/ownership/<ticker>')
@login_required
def api_beta_ownership(ticker):
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403
    return jsonify(get_ownership_data(ticker))

@app.route('/api/beta/anomalies')
@login_required
def api_beta_anomalies():
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403
...
@app.route('/api/beta/rebalance', methods=['POST'])
@login_required
def api_beta_rebalance():
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403

    data = request.get_json()
    holdings = data.get('holdings', [])
    targets = data.get('target_weights', {})
    equity = data.get('total_equity', 0)

    if not holdings or not targets:
        return jsonify({'error': 'Missing holdings or targets'}), 400

    orders = generate_rebalancing_orders(holdings, targets, equity)
    return jsonify(orders)

@app.route('/api/beta/yield-curve')
@login_required
def api_beta_yield_curve():
    if not current_user.beta_active:
        return jsonify({'error': 'Beta features not active.'}), 403
    return jsonify(get_yield_curve())
